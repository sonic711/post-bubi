...
## 4.3 後續擴充：gRPC BUR 組包請求

依 `data/[SIT]FAS_MOCK_NM001_S00.ipynb` 的腳本概念，後續可新增 `gRPC BUR` request type。此模式與一般 gRPC JSON request 不同，使用者只需關注目標系統 host / port 與業務 input 內容，系統負責完成 TCPIP header、MCS header、Basic Label、Text Area 串接、UTF-8 / BUR 轉碼、`RqPayload` 建立與 `Service/rpcPeriphery` 呼叫。
